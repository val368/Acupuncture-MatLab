...
pixel_location = [  75    35    15    99    69    43    55;
...
while true
    ...
    axis image
    colorbar();
    title("press SPACEBAR to proceed to data acquisition")
    for label = 1:7
        y1 = pixel_location(1,label);
        y2 = pixel_location(1,label) + 20;
        x1 = pixel_location(2,label);
        x2 = pixel_location(2,label) + 20;
        line([y1,y1],[x1 x2],'color',channel_text(label));
        line([y2,y2],[x1 x2],'color',channel_text(label));
        line([y1,y2],[x1 x1],'color',channel_text(label));
        line([y1,y2],[x2 x2],'color',channel_text(label));
    end
    pause(0.1)
...
for j = 1:N
    ...
    for label = 1:ch_len    % BFI calculation for selected channels
        y1 = pixel_location(1,label);
        y2 = pixel_location(1,label) + 20;
        x1 = pixel_location(2,label);
        x2 = pixel_location(2,label) + 20;
        samp_img = img(y1:y2,x1:x2);
        img_array{label}(:,:,j) = samp_img;

        BFI_box = zeros(9,1);
        y = [1 8 15];
        x = [1 8 15];
        [Y,X] = meshgrid(y,x);
        Y_pixel = reshape(Y, [9 1]);
        X_pixel = reshape(X, [9 1]);

        for co = 1 : 9
            MEAN = mean(samp_img(Y_pixel(co):Y_pixel(co)+6,X_pixel(co):X_pixel(co)+6),"all");
            STD = std(samp_img(Y_pixel(co):Y_pixel(co)+6,X_pixel(co):X_pixel(co)+6),1,'all');
            K = STD/MEAN;
            BFI_box(co) = 1/K^2;
        end
    ...
    if rem(j,10) == 0
        ...
        median_location = [median_location mean(final_BFI(label,j-9:j),'all')];

        plot(frame_label,final_BFI(label,:),'.','Color',ch(label))
        plot(mean_bfi(1,:),median_location(1,:),ch(label),'LineWidth',3);
        hold on
        ...
        axis([-100 (totalframe+100) 0 max(final_BFI(label,:))+ max(final_BFI(label,:))*0.2])
    end
    end
